...
{
    /// <summary>
    /// Dialog JSON Settings — editor full-width với Format, Validate, Save.
    /// Mở từ Tools menu > "Async Tool Window Sample Settings (JSON)..."
    /// Dùng DTO result classes (JsonParseResult, JsonFormatResult, JsonSaveResult)
    /// thay vì ValueTuple để tương thích C# 7.0 / .NET 4.6.
    /// </summary>
    public partial class JsonSettingsDialog : Window
    {
        ...
        private void BtnFormat_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (!result.IsOk)
            {
                SetStatusError("✘  Cannot format: " + result.Error);
                return;
            }

            // Preserve caret line approximately
        ...
        private void BtnReset_Click(object sender, RoutedEventArgs e)
        {
            MessageBoxResult answer = MessageBox.Show(
                "Reset cấu hình về mặc định?\nFile JSON hiện tại sẽ bị ghi đè.",
                "Confirm Reset",
        ...
        private void BtnCopy_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                SetStatusInfo("📋  JSON copied to clipboard.");
            }
            catch (Exception ex)
            {
                SetStatusError("✘  Copy failed: " + ex.Message);
            }
        }
        ...
        private void BtnPaste_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    SetStatusError("✘  Clipboard is empty.");
                    return;
                }

                // Auto-format after paste if valid JSON
                JsonFormatResult fmt = _jsonConfig.FormatJson(clip);
                LoadJsonIntoEditor(fmt.IsOk ? fmt.Result : clip);

                if (fmt.IsOk)
                    SetStatusOk("✔  Pasted and formatted from clipboard.");
                else
                    SetStatusInfo("📌  Pasted from clipboard (not valid JSON yet).");

                _outputWindow.Log("[JsonConfig] Pasted from clipboard.");
            }
            catch (Exception ex)
            {
                SetStatusError("✘  Paste failed: " + ex.Message);
            }
        }

        // ================================================================== //
        //  Live text change — update counter + quick syntax check            //
        // ================================================================== //
        ...
        private void TxtJson_TextChanged(object sender, System.Windows.Controls.TextChangedEventArgs e)
        {
            ...
            {
                SetStatusError("✘  Editor is empty.");
                return;
            }

            // Quick brace-count check while typing
            int open = 0, close = 0;
        ...
        private void BtnSave_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                SetStatusError("✘  " + result.Error);
                _outputWindow.Log("[JsonConfig] Save FAILED: " + result.Error);
                return;
            }
            ...
            string summary = result.ChangedKeys.Count == 0
                ? "No changes."
                : result.ChangedKeys.Count + " key(s) changed: "
                  + string.Join(", ", result.ChangedKeys);
